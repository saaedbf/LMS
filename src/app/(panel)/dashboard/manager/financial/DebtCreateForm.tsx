...
export function DebtCreateForm({ payes, klasses, onCreated, setOpen }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      const res = await createDebtsAction({
      ...
      if (res.status === "error") {
        ...
        setMessage({
          type: "success",
          text: `بدهکاری برای ${res.data?.created} دانش‌آموز ثبت شد.`,
        });
        resetForm();
        onCreated?.();
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-base font-bold text-zinc-800">ثبت بدهکاری جدید</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          >
            {isPending
              ? "در حال ثبت..."
              : `ثبت بدهکاری برای ${selectedEnrollmentIds.length} دانش‌آموز`}
          </button>
        </div>
      </form>
    </div>
