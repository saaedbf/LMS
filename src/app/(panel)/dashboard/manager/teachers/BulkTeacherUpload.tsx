...
import { bulkCreateTeachers } from "@/actions/teacherBulkActions";
...
export default function BulkTeacherUpload({
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, "نمونه-ثبت-گروهی-معلمان.xlsx");
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const rows: ParsedRow[] = jsonData.map((row, idx) => ({
        ...
        personnelCode: String(row["کد پرسنلی"] || "").trim(),
  ...
  const validateRows = () => {
    return parsedRows.map((row) => {
      ...
      if (!row.phone) errors.push("شماره تماس الزامی است");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === "error") {
        ...
        setResult(res.data);
        toast.success(`${res.data.created} معلم با موفقیت ثبت شد`);
    ...
    <div className="space-y-1 p-2  max-h-[80vh]">
      ...
      <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-blue-900">ثبت گروهی معلمان</p>
            <p className="mt-1 text-xs text-blue-700">
              معلمان به صورت خودکار به این مدرسه و سال تحصیلی منتسب می‌شوند.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex flex-1 items-center justify-center gap-2 rounded-lg border-2 p-2 text-sm font-medium transition-all ${
            mode === "upload"
      ...
      {mode === "paste" && (
        <div>
          ...
          <textarea
            ...
            rows={3}
            placeholder={`نام\tنام خانوادگی\tکد ملی\tکد پرسنلی\tشماره تماس
علی\tمحمدی\t0012345678\t12345\t09123456789`}
          ...
          <button
            type="button"
            onClick={handlePaste}
            className="mt-2 inline-flex items-center gap-1 rounded-lg bg-blue-600 px-4 py-1 text-sm font-medium text-white hover:bg-blue-700"
          >
