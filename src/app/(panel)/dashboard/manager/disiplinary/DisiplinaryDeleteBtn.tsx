...
import { deleteDisiplinaryAction } from "@/actions/disciplinaryActions";
...
export default function DisiplinaryDeleteBtn({ absenceId }: Props) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      try {
        ...
        const result = await deleteDisiplinaryAction({ id: absenceId });

        if (result.status === "error") {
          toast.error(result.error.toString() || "خطا در حذف ");
        } else {
        ...
        >
          ...
          >
            ...
            <p className="mt-2 text-xs text-zinc-600 dark:text-zinc-300">
              آیا از حذف این رکورد مورد انضباطی مطمئن هستید؟ این عملیات قابل
              بازگشت نیست.
            </p>
