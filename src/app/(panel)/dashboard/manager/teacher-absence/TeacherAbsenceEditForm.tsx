...
import { updateTeacherAbsenceAction } from "@/actions/teacherAbsenceActions";
...
export default function TeacherAbsenceEditForm({ absence, setOpen }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateTeacherAbsenceAction({
        id: absence.id,
        reason: reason.trim() || null,
      });

      if (res.status === "error") {
        toast.error(res.error.toString() || "خطا در ویرایش");
      } else {
        toast.success("علت غیبت با موفقیت ویرایش شد");
        setOpen(false);
      }
    });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4 p-4 text-sm">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="مرخصی استحقاقی، مأموریت، بیماری و ..."
